...
do $$ begin
  ...
  if not found then
    create policy "Profiles: staff manage" on public.profiles
      for all using (public.is_staff((select auth.uid()))) with check (public.is_staff((select auth.uid())));
  end if;
end $$;
...
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='user_roles' and policyname='UserRoles: admin manage') then
    create policy "UserRoles: admin manage" on public.user_roles
      for all using (public.has_role((select auth.uid()),'admin')) with check (public.has_role((select auth.uid()),'admin'));
  end if;
end $$;
...
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='pending_registrations' and policyname='Pending: staff read/update') then
    create policy "Pending: staff read/update" on public.pending_registrations for all using (public.is_staff((select auth.uid()))) with check (public.is_staff((select auth.uid())));
  end if;
end $$;
...
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='events' and policyname='Events: self read/insert') then
    create policy "Events: self read/insert" on public.events for select using ((select auth.uid()) = user_id);
    create policy "Events: self insert" on public.events for insert with check ((select auth.uid()) = user_id);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='events' and policyname='Events: staff read') then
    create policy "Events: staff read" on public.events for select using (public.is_staff((select auth.uid())));
  end if;
end $$;
...
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='forum_topics' and policyname='ForumTopics: read') then
    ...
    create policy "ForumTopics: create" on public.forum_topics for insert with check ((select auth.uid()) is not null);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='forum_posts' and column_name='author_id') then
    ...
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='forum_posts' and policyname='ForumPosts: read') then
      ...
      create policy "ForumPosts: create" on public.forum_posts for insert with check ((select auth.uid()) = user_id);
    end if;
  else
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='forum_posts' and policyname='ForumPosts: read') then
      create policy "ForumPosts: read" on public.forum_posts for select using (true);
      create policy "ForumPosts: create" on public.forum_posts for insert with check ((select auth.uid()) = author_id);
    end if;
  end if;
end $$;
...
-- Drop either materialized view or view variant if present (idempotent across states)
do $$ begin
  if exists (
