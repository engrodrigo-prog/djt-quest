-- Fix: campaign evaluations must always assign two evaluators (primary + cross).
-- Also cleanup any pending evaluation_queue rows with NULL assignee that can leave items "sem líder atribuído".
...
begin
  ...
  -- Default logic (primary/leader + cross), aligned with parallel evaluations.
  ...
  -- Leaders are detected by profiles.is_leader OR roles (lider_equipe/coordenador/gerente/admin).
  select p0.id into c_immediate
  ...
  -- If no immediate leader exists in the hierarchy, distribute by picking any leader as primary.
  if c_immediate is null then
    with candidates0 as (
      ...
      where p3.id <> _submitter
        ...
        and not exists (
          ...
          where eq3.event_id = _event_id
            and eq3.assigned_to = p3.id
            and eq3.completed_at is null
        )
    )
    select user_id into c_immediate
    from candidates0
  ...
  -- If we only need one evaluation, stop here.
  if not requires_two then
  ...
  -- If we already have two assignees, stop here.
  if existing_assignees >= 2 then
    return;
  end if;
  ...
  -- Ensure we reach two assignees whenever possible.
  loop
    select count(distinct assigned_to)
      into existing_assignees
      from public.evaluation_queue eq
...
-- Best-effort backfill: if Rodrigo has a pending campaign item with NULL assignee, re-run assignment.
do $$
declare
  rodrigo_id uuid;
  ev record;
...
begin
  ...
  if rodrigo_id is null then
    raise notice 'Dispatch (campaign null assignee): Rodrigo profile not found; skipping.';
    return;
  end if;
  ...
  raise notice 'Dispatch (campaign null assignee): processed=%', processed;
end $$;
