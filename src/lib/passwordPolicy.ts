...
export const validateNewPassword = (passwordRaw: string, confirmRaw: string) => {
  ...
  if (!/[a-z]/.test(password) || !/[A-Z]/.test(password) || !/\d/.test(password) || !/[^A-Za-z0-9]/.test(password)) {
    return {
      ok: false as const,
      message: "Use pelo menos 1 maiúscula, 1 minúscula, 1 número e 1 símbolo",
    };
  }
  if (password !== confirm) {
...
export const mapPasswordUpdateError = (error: unknown): PasswordErrorInfo => {
  ...
  const msg =
    error instanceof Error
      ...
      : typeof anyError?.message === "string"
        ...
        : typeof anyError?.raw?.message === "string"
          ? String(anyError.raw.message)
        : "";
  ...
  if (
    code.includes("reauth_required") ||
    lower.includes("reauthentication required") ||
    lower.includes("secure password change")
  ) {
    return {
      title: "Confirmação de segurança necessária",
      description: "Faça login novamente e tente trocar a senha com uma sessão recente.",
    };
  }

  if (
    lower.includes("one character of each") ||
    ...
    return {
      ...
      description: "Use maiúscula, minúscula, número e símbolo (ex.: !@#$%).",
    };
  }
  ...
  if (
    ...
    return {
      ...
      description: msg || "Use uma senha forte (8+ caracteres, maiúscula, minúscula, número e símbolo).",
    };
  }
