...
const Navigation = () => {
  ...
  useEffect(() => {
    ...
    const recalcDesktopScale = () => {
      ...
      const fixedCost = 84;
      const rowBaseHeight = 62;
      const rawScale = (viewportHeight - fixedCost) / (totalRows * rowBaseHeight);
      const nextScale = Math.max(0.76, Math.min(1.18, Number.isFinite(rawScale) ? rawScale : 1));
  ...
  const bubbleClass = (active: boolean, size: 'sm' | 'md' | 'lg' | 'xl' = 'md') =>
    cn(
      ...
      size === 'xl'
        ? 'h-[calc(50px*var(--djt-nav-scale,1))] w-[calc(50px*var(--djt-nav-scale,1))]'
        : size === 'lg'
          ? 'h-[52px] w-[52px]'
          : size === 'md'
            ? 'h-10 w-10'
      ...
      >
        <div className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-950/88 p-1.5 shadow-2xl backdrop-blur-xl">
          ...
          <div className="mb-0.5 flex items-center justify-between">
            <p className="pl-1 font-semibold tracking-wide text-slate-100" style={{ fontSize: `${Math.max(12, Math.round(15 * desktopNavScale))}px` }}>
              Menu
            </p>
            {navHidden && (
              <Button
            ...
            <div className="space-y-px pb-0">
              {desktopItems.map((item) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1 truncate font-semibold leading-4" style={{ fontSize: `${Math.max(12, Math.round(13 * desktopNavScale))}px` }}>
                    {item.label}
                  </span>
                </button>
              ))}
