...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (updates && typeof updates === 'object') {
            if (typeof updates.matricula === 'string')
                payload.matricula = updates.matricula.trim();
            ...
            if (typeof updates.sigla_area === 'string')
                payload.sigla_area = updates.sigla_area.trim().toUpperCase();
            if (typeof updates.operational_base === 'string')
                payload.operational_base = updates.operational_base.trim();
        }
        if (Object.keys(payload).length === 0)
            return res.status(400).json({ error: 'Nenhuma atualização válida recebida' });
        let targetId = userId;
        if (!targetId && email) {
