...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (reqAuthErr || !requesterId) return res.status(401).json({ error: 'Unauthorized' });
    ...
    if (isGuest) {
      ...
      try {
        ...
        if (!existing?.id) {
          await admin.from('teams').insert({ id: teamId, name: teamId } as any);
        }
      } catch {}
    }
    ...
    if (!newUserId) return res.status(400).json({ error: 'Falha ao criar usuário' });
