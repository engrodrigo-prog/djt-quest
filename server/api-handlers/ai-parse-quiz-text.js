...
const clampQuestions = (raw) => {
  const n = Number(raw);
  if (!Number.isFinite(n)) return 30;
  return Math.max(1, Math.min(Math.floor(n), 50));
};

const isDifficulty = (v) => ['basico', 'intermediario', 'avancado', 'especialista'].includes(String(v || '').trim());
...
const heuristicParse = (text) => {
  ...
  for (const block of blocks) {
    ...
    for (const line of lines) {
      ...
      } else if (!/^correta\b|^resposta\b/i.test(line)) {
...
const sanitizeQuestions = ({ questions, defaultDifficulty = 'intermediario' }) => {
  const issues = [];
  const cleaned = [];
  ...
  for (let i = 0; i < questions.length; i += 1) {
    ...
    if (optsRaw) {
      ...
      // Legacy object shape (alt_a ... correta)
...
- Retorne no máximo ${limit} perguntas.`;
        ...
        const body = {
          ...
          ...(isGpt5 ? { max_completion_tokens: 2600 } : { max_tokens: 2600, temperature: 0.2 }),
        };
