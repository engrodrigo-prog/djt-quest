...
export default async function handler(req: any, res: any) {
  ...
  const eventId =
    (typeof req.query?.event_id === 'string' ? req.query.event_id : undefined) ||
  ...
  const { data: eventRow } = await admin
    ...
    .eq('id', eventIdClean)
    .maybeSingle()
  if (!eventRow?.id) return res.status(404).json({ error: 'Evento não encontrado' })
  ...
  if (!allowed) {
    try {
      if (useServiceRole) {
        ...
        if (!error && staffFlag) allowed = true
      }
    } catch {
      // ignore
    }
  ...
  const leaderIds = Array.from(
    new Set((assignments || []).map((r: any) => String(r?.assigned_to || '')).filter(Boolean)),
  )
  const participantIds = Array.from(
    new Set((participantsRows || []).map((r: any) => String(r?.user_id || '')).filter(Boolean)),
  ).filter((id) => id !== String(eventRow.user_id))
