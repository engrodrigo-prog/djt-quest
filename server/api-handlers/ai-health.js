...
let lastHealthCache = null;

const probeOpenAiModelAccess = async ({ key, model, timeoutMs }) => {
...
const cacheTtlForPayload = (payload) => (payload?.ok ? CACHE_TTL_OK_MS : CACHE_TTL_ERR_MS);
...
const getCachedResult = (forceRefresh) => {
...
export default async function handler(req, res) {
  ...
  try {
    const forceRefreshRaw = Array.isArray(req?.query?.force) ? req.query.force[0] : req?.query?.force;
    ...
    if (!key) {
      return res.status(200).json(cacheResult({ ok: false, stage: 'env', error: 'OPENAI_API_KEY ausente', latency_ms: Date.now() - started }));
    }
