...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!enabled)
            return res.status(200).json({ success: true, message: 'Forum bonus disabled' });
        // Compute month range
        const now = new Date();
        ...
        if (!filtered.length)
            return res.status(200).json({ success: true, message: 'No forum scores for this month' });
        ...
        for (const e of (events || [])) {
            ...
            if (v > 0)
                baseMap[e.user_id] = (baseMap[e.user_id] || 0) + v;
        }
        const awards = [];
        ...
        if (!awards.length)
            return res.status(200).json({ success: true, message: 'No eligible users for bonus' });
        // Upsert awards (idempotent), then apply XP
        const { error: awErr } = await admin.from('forum_monthly_bonus_awards').upsert(awards, { onConflict: 'month,user_id' });
        if (awErr)
