...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (!SUPABASE_URL || !SERVICE_ROLE) return res.status(500).json({ error: 'Missing Supabase server config' })
    ...
    if (req.method === 'POST') {
      if (!ALLOWED_CREATORS.has(email)) return res.status(403).json({ error: 'Somente planejamento/gerência podem criar' })
      const body = req.body || {}
      const {
        title,
        description = null,
        ...
        allow_early = true,
        allow_late = false,
        target_roles = ['coordenador_djtx','gerente_divisao_djtx','gerente_djt'],
      } = body

      if (!title || !due_date) return res.status(400).json({ error: 'title e due_date são obrigatórios' })
      ...
      const { data: challenge, error: chErr } = await admin
        ...
        .select()
        .single()
      if (chErr || !challenge) return res.status(400).json({ error: chErr?.message || 'Falha ao criar desafio' })
      ...
      const uniqueUsers = Array.from(new Set((leaders || []).map((r: any) => r.user_id)))
    ...
    if (req.method === 'PATCH') {
      if (!ALLOWED_CREATORS.has(email)) return res.status(403).json({ error: 'Somente planejamento/gerência' })
