...
const getEffectiveRole = (roles, isLeader) => {
    ...
    if (set.has('admin'))
        return 'admin';
    if (set.has('gerente_djt'))
        return 'gerente_djt';
    if (set.has('gerente_divisao_djtx'))
        return 'gerente_divisao_djtx';
    if (set.has('coordenador_djtx'))
        return 'coordenador_djtx';
    if (set.has('lider_equipe') || isLeader)
        return 'lider_equipe';
    return null;
};
export default async function handler(req, res) {
    ...
    try {
        ...
        let q = admin.from('pending_registrations').select('*').order('created_at', { ascending: false }).limit(500);
        const ors = [];
        if (role === 'admin' || role === 'gerente_djt') {
            ...
            if (divisionId)
                ors.push(`sigla_area.ilike.${divisionId}%`);
            ...
            if (divisionId)
                ors.push(`sigla_area.ilike.${divisionId}%`);
            if (coordId)
                ors.push(`sigla_area.ilike.${coordId}%`);
            if (teamId)
                ors.push(`sigla_area.eq.${teamId}`);
            ors.push(`sigla_area.eq.${GUEST_TEAM_ID}`);
            q = q.or(ors.join(','));
        }
