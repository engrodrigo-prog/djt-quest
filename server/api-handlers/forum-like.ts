...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === 'like') {
      await authed.from('forum_likes').upsert({ post_id, user_id: uid } as any).throwOnError()
    } else {
      const deleter = SERVICE_ROLE_KEY ? admin : authed
      await deleter.from('forum_likes').delete().eq('post_id', post_id).eq('user_id', uid).throwOnError()
    }
